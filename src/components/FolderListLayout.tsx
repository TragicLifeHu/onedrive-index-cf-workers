          ...
          {c.folder ? (
            ...
            <div className="hidden p-1.5 text-gray-700 md:flex dark:text-gray-400">
              <span
                ...
                onClick={() => {
                  clipboard.copy(
                    `${getBaseUrl()}/api/raw?path=${encodeURIComponent(getItemPath(c.name))}${hashedToken ? `&odpt=${hashedToken}` : ''}`,
                  )
              ...
              <a
                title={'Download file'}
                className="cursor-pointer rounded px-1.5 py-1 hover:bg-gray-300 dark:hover:bg-gray-600"
                href={`/api/raw?path=${encodeURIComponent(getItemPath(c.name))}${hashedToken ? `&odpt=${hashedToken}` : ''}`}
              >
                <FontAwesomeIcon icon={['far', 'arrow-alt-circle-down']} />
              </a>
            </div>
